...
const Stats: React.FC = () => {
  const { loading: packagesLoading, data: packageData } = useQuery<PackageData>(GET_PACKAGES_QUERY);

  const filteredPackages = useMemo(() => {
    if (!packageData) return [];
    return packageData.getAllPackages.filter(pkg => 
      !['REFUNDED', 'CANCELLED', 'BACK'].includes(pkg.status)
    );
  }, [packageData]);
  ...
  const packageStats = useMemo(() => {
    ...
    return filteredPackages.reduce<PackageStats>((stats, pkg) => {
  ...
  const statCards: Array<{ title: string; period: keyof Omit<PackageStats, 'byStatus'> }> = [
  ...
  return (
    <div className="bg-gray-100 p-6 rounded-lg shadow-md relative">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Tableau de bord des commandes</h2>
          ...
          <div key={period} className="bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            <div className="p-4">
              {packagesLoading ? (
                ...
                <>
                  <p className="text-3xl font-bold text-gray-800">{packageStats[period].total.toFixed(2)} DT</p>
                  <p className="text-sm text-gray-600 mt-1">
                    {packageStats[period].count} Commande{packageStats[period].count !== 1 ? 's' : ''}
                  </p>
                </>
              )}
            </div>
